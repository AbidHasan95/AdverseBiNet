...
path = 'gdrive/My Drive/binary_imgs/data2'
...
params = {'path' : path,
          ...
          'beta_1': 0.5,
          'epochs': 50,
          'Img_saved_path' : 'gdrive/My Drive/binary_imgs/SavedImgs',
          'Img_saved_path_for_real_data' : 'gdrive/My Drive/binary_imgs/SavedReal',
          'Stage_epochs' : [800,1000]}
...
def get_file_paths(path):
    ...
    #gt_path = [os.path.join(root, file)  for root, dirs, files in os.walk(path) for file in files if '_gt' in file]
...
def get_style_loss(output, style):
    ...
    data_dict = loadWeightsData('gdrive/My Drive/binary_imgs/vgg/vgg16.npy')
...
for epoch in range(params['epochs']):
    ...
    for idx in range(data_set_size//params['batch_size']):
        ...
        counter = counter + 1
        if counter%400==0 or counter>=1498:
